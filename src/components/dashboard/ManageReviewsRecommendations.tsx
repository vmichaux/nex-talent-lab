...
export function ManageReviewsRecommendations({ role = "talent" }: ManageReviewsRecommendationsProps) {
      ...
      <Card className="shadow-sm">
        <CardContent className="p-0">
          {reviews.slice(0, 3).map((review) => (
            <div 
              key={review.id} 
              className="flex flex-col p-3 border-b last:border-b-0 hover:bg-gray-50 transition-colors"
            >
              ...
              <div className="flex mt-1">
                {Array(review.rating).fill(0).map((_, i) => (
                  <ThumbsUp key={i} className="h-3 w-3 text-yellow-500 fill-yellow-500" />
                ))}
              </div>
            </div>
          ))}
