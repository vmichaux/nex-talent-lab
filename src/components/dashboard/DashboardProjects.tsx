...
export function DashboardProjects() {
  ...
  return <div>
      ...
      {loading ? <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1, 2, 3].map(skeleton => <Card key={skeleton} className="overflow-hidden shadow-sm">
              <CardHeader>
          ...
          {displayedProjects.length > 0 ? displayedProjects.map(project => <Card key={project.id} className="overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                ...
                <CardContent className="px-4 pb-3 pt-0 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {project.skills && project.skills.slice(0, 2).map((skill, index) => <span key={index} className="bg-primary/10 text-primary text-[10px] px-2 py-1 rounded-full">
                        {skill}
                      </span>)}
                  </div>
